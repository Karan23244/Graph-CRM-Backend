  ...
  try {
    ...
    const [eligiblePids] = await pool.query(`
      ...
      FROM (
        SELECT pid, COUNT(DISTINCT campaign_name) AS campaign_count
        FROM campaign_metrics
        WHERE metrics_date >= DATE_FORMAT(CURRENT_DATE(), '%Y-%m-01')
    ...
    if (eligiblePids.length === 0) {
      console.log("No PIDs with >=5 campaigns this month");
      return res.json([]);
    }

    const pidList = eligiblePids.map((row) => row.pid);

    // 2️⃣ Fetch accumulated metrics per PID+Campaign
    const [metrics] = await pool.query(
      `
      SELECT 
        pid,
    ...
    for (const m of metrics) {
      ...
      // zone evaluation with ignore flags applied
      const zone = getZoneDynamic(fraud, cti, ite, etc, cond);
    ...
    // 5️⃣ Filter PIDs where >60% campaigns are Red/Orange
    const alerts = Object.entries(pidStats)
      .filter(([_, stats]) => {
        const redOrangePct = (stats.redOrOrange / stats.total) * 100;
        return redOrangePct > 60;
    ...
    console.error("❌ /pids-on-alert error:", err);
    res.status(500).json({ error: "DB error" });
  }
});
...
router.get("/pids-stable", async (req, res) => {
  try {
    // 1️⃣ Get only PIDs with at least 5 campaigns in the current month
    const [eligiblePids] = await pool.query(`
      SELECT pid
      FROM (
        SELECT pid, COUNT(DISTINCT campaign_name) AS campaign_count
    ...
    // 2️⃣ Fetch accumulated monthly metrics per PID+Campaign
    const [metrics] = await pool.query(
      `
      SELECT 
        pid,
    ...
    console.error("❌ /pids-stable error:", err);
    res.status(500).json({ error: "DB error" });
  }
});
